...
const uint8_t ALPHABET_MAP[] = {0x88, 0x83, 0xC6, 0xA1, 0x86, 0x8E, 0xC2,
                                0x89, 0xCF, 0xE1, 0x89, 0xC7, 0xEA, 0xC8,
...
void writeCharToSegment(uint8_t segment, char character){
  uint8_t chosenLetter;
  int newCharacter = (int)character;
  if(newCharacter >= 97){
    newCharacter-=32;
  }

  if(newCharacter == 95) { // ASCII value of '_'
      chosenLetter = ALPHABET_MAP[26];
  } else if(newCharacter >= 65 && newCharacter <= 90) {
      chosenLetter = ALPHABET_MAP[newCharacter - 65];
  } else {
      chosenLetter = 0xFF;
  }

  cbi(PORTD, LATCH_DIO);
  shift(chosenLetter, MSBFIRST);
  shift(SEGMENT_SELECT[segment], MSBFIRST);
  sbi(PORTD, LATCH_DIO);
